...
class ReplayBuffer:
    ...
    def update(self, abs_td_err, index):
        ...
        """
        td_updated = abs_td_err.squeeze() #64,
    ...
    def sample_t(self, p_replay_beta):
        ...
        weight = np.empty((self.batch_size, 1), dtype=np.float32)
        ...
        max_weight = (p_min * self.batch_size) ** (-p_replay_beta)
        ...
        for i in range(n):
            ...
            weight[i, 0] = np.power(self.batch_size * sampling_probabilities, -p_replay_beta)/ max_weight
        ...
        weight = torch.from_numpy(np.mean(weight)).float() #change form
